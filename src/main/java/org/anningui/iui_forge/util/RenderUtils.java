...
public final class RenderUtils {
    ...
    public static void fillRoundShadow(GuiGraphics context, int x, int y, int w, int h, int r, int thickness, int innerColor, int outerColor) {
        ...
        buf.vertex(mat, corners[0][0], y, 0).color(innerColor).endVertex(); // connect last to first vertex
        buf.vertex(mat, corners[0][0], y - thickness, 0).color(outerColor).endVertex(); // connect last to first vertex
    ...
    public static void drawRoundComponenture(GuiGraphics context, ResourceLocation texture, int x, int y, int w, int h, int r) {
        ...
        buf.vertex(mat, x + w / 2F, y + h / 2F, 0).uv(0.5F, 0.5F).endVertex();
        ...
        for (int corner = 0; corner < 4; corner++) {
            ...
            for (int i = cornerStart; i <= cornerEnd; i += 10) {
                ...
                float u = (rx - x) / w;
                float v = (ry - y) / h;
                buf.vertex(mat, rx, ry, 0).uv(u, v).endVertex();
            }
        }

        buf.vertex(mat, corners[0][0], y, 0).uv(((float)corners[0][0] - x) / w, 0).endVertex(); // connect last to first vertex
