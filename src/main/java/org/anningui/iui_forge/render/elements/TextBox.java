...
import org.anningui.iui_forge.render.constants.BackgroundClip;
import org.anningui.iui_forge.render.constants.Visibility;
    ...
    public void onRender(GuiGraphics context, int mx, int my, float delta) {
        ...
        if (visibility != Visibility.ONLY_CHILDREN) {
            ...
            if (parentPanel != null) {
                ...
                Component display = Text.of(text);
                if (!queryMatchesPattern())
                    ...
                    RenderUtils.drawDefaultScaledComponent(context, Text.of(getDefaultText()), x + height / 2 + 2, y + height / 3, 0.9F, false, textColor.darker().darker().getHex());
